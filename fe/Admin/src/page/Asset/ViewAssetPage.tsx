...
const ViewAssetPage = () => {
  ...
  const handleMaintenance = async (row: any) => {
    try {
      ...
      const message = response?.data?.message || t("asset.maintenanceSuccess");
      showAlert({ title: message, type: "success", autoClose: 3000 });
      fetchData();
    } catch (err: any) {
      showAlert({
        title: err?.response?.data?.message || t("asset.maintenanceError"),
        type: "error",
      });
    }
  };
  ...
  const handleActive = async (row: any) => {
    try {
      ...
      const message = response?.data?.message || t("asset.activeSuccess");
      showAlert({ title: message, type: "success", autoClose: 3000 });
      fetchData();
    } catch (err: any) {
      showAlert({
        title: err?.response?.data?.message || t("asset.activeError"),
        type: "error",
      });
    }
  };
  const handleDeActived = async (row: any) => {
    try {
      ...
      const message = response?.data?.message || t("asset.deactiveSuccess");
      showAlert({ title: message, type: "success", autoClose: 3000 });
      fetchData();
    } catch (err: any) {
      showAlert({
        title: err?.response?.data?.message || t("asset.deactiveError"),
        type: "error",
      });
    }
  };
  ...
  const columns = [
...

    {
      ...
      render: (row: any) => {
        ...
        > = {
          1: {
            label: t("asset.status.good"),
            color: "bg-green-50 text-green-700",
            dot: "bg-green-500",
          },
          2: {
            label: t("asset.status.maintenance"),
            color: "bg-red-50 text-red-700",
            dot: "bg-red-500",
          },
          3: {
            label: t("asset.status.broken"),
            color: "bg-fuchsia-50 text-fuchsia-700",
            dot: "bg-fuchsia-500",
          },
          4: {
            label: t("asset.status.deactivated"),
