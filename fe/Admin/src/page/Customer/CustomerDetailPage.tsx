...
const CustomerDetailPage = () => {
  ...
  const bookingColumns = [
    {
      ...
      sortable: true,
      render: (row: any) => (
        <span className="font-medium text-[#2E3A8C]">{row.code}</span>
      ),
    },

    {
      ...
      render: (row: any) =>
        row.checkInDate
          ? new Date(
              `${row.checkInDate}T${row.checkInTime || "00:00:00"}`,
...

    {
      ...
      render: (row: any) =>
        row.checkOutDate
          ? new Date(
              `${row.checkOutDate}T${row.checkOutTime || "00:00:00"}`,
            ).toLocaleString("vi-VN")
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#111827] p-4 sm:p-6 space-y-6 transition-colors">
      {/* ================= BACK ================= */}
      <button
        onClick={() => navigate("/Dashboard/customer")}
        className="flex items-center gap-2 text-sm text-[#2E3A8C] dark:text-blue-400 hover:underline"
      >
      ...
      {/* ================= HEADER ================= */}
      <div className="bg-white dark:bg-[#1F2937] dark:border dark:border-gray-700 rounded-xl shadow p-4 sm:p-6 space-y-4 transition-colors">
          ...
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${
              customer.blocked
                ...
                : "bg-[#EEF1FF] text-[#2E3A8C] dark:bg-blue-900/30 dark:text-blue-400"
            }`}
          >
            {customer.blocked
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 text-gray-600 dark:text-gray-300 text-sm">
          <div className="flex items-center gap-2">
            <Phone className="w-4 h-4 text-[#2E3A8C] dark:text-blue-400" />
            {customer.phone}
          </div>

          <div className="flex items-center gap-2 break-all">
            <Mail className="w-4 h-4 text-[#2E3A8C] dark:text-blue-400" />
            {customer.email}
          </div>
        </div>
      ...
      {/* ================= TABS ================= */}
      <div className="flex gap-6 border-b border-gray-300 dark:border-gray-700 overflow-x-auto whitespace-nowrap">
        {["basic", "points"].map((tab) => (
          <button
            ...
            className={`pb-2 transition ${
              activeTab === tab
                ? "border-b-2 border-[#2E3A8C] dark:border-blue-400 text-[#2E3A8C] dark:text-blue-400 font-semibold"
      ...
      {activeTab === "basic" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* INFO */}
            <div className="bg-white dark:bg-[#1F2937] dark:border dark:border-gray-700 rounded-xl shadow p-4 sm:p-6 space-y-4 text-sm">
              {[
                ...
                <div key={key} className="flex justify-between gap-4">
                  ...
                  <span className="font-medium break-all text-right text-gray-900 dark:text-white">
                    {value}
                  </span>
                </div>
              ))}
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <StatCard
                  icon={
                    <Wallet className="w-4 h-4 text-[#2E3A8C] dark:text-blue-400" />
                  }
                  label={t("customer.points.total")}
                  value={customer.totalPoint}
                  color="text-[#2E3A8C] dark:text-blue-400"
                />
              ...
              rounded-xl p-4 flex gap-3 text-sm 
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
            {/* TABLE */}
            <div
              className="xl:col-span-3 bg-white dark:bg-[#1F2937]
            dark:border dark:border-gray-700
            rounded-xl shadow p-4 sm:p-6"
            >
            ...
            dark:border dark:border-gray-700
            rounded-xl shadow p-4 sm:p-6 flex flex-col gap-6"
            >
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">
      ...
      {activeTab === "points" && (
        <div
          className="bg-white dark:bg-[#1F2937]
        dark:border dark:border-gray-700
        rounded-xl shadow p-4 sm:p-6 space-y-4"
        >
          {/* FILTER */}
          <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
            <select
              ...
              className="w-full lg:w-64 border border-gray-200 dark:border-gray-600
