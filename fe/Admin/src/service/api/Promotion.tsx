...
export const updatePromotion = async (promotionId: number, promotionData: any) => {
  return await Http.put(`/api/promotions/${promotionId}`, promotionData);
}

export const updateStatusPromotion = async (promotionId: number, isActive: any) => {
  return await Http.patch(`/api/promotions/${promotionId}/status?status=${isActive}`);
}