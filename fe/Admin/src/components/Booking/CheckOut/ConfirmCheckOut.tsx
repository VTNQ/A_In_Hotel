...
import type { ConfirmCheckOutProps, ExtraCharge } from "../../../type/booking.types";
...
import { handleCheckOut } from "../../../service/api/Booking";
...
const ConfirmCheckOut = ({
    ...
    useEffect(() => {
        ...
        const fetchExtraService = async () => {
            try {
                ...
                if (usedExtraServiceIds.length > 0) {
                    filterParts.push(
                        `id=out=(${usedExtraServiceIds.join(",")})`
                    );
                }
                ...
                setExtraService(response.data?.content || []);
            } catch (err) {
                console.log(err);
            }
        };
        ...
        fetchExtraService();
    }, [open, data, usedExtraServiceIds.join(",")]);
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            await handleCheckOut(data.id, payload);
            onConfirm();
            onCancel();
        } finally {
            setConfirming(false);
    ...
    const serviceOptions = extraService.map((s) => ({
        value: s.id,
        label: s.serviceName,
    }));
    ...
    const outstanding =
        totalCharges - (data.payment[0]?.paidAmount ?? 0);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col max-h-[90vh] ring-1 ring-black/5">
                ...
                {/* ================= BODY ================= */}
                <div className="p-6 overflow-y-auto custom-scroll space-y-6 bg-white">
                    ...
                    <div className="bg-[#f6f8fb] rounded-xl p-5 border border-[#d6dbea]">
                        <h2 className="font-bold text-sm text-[#0E5E6F] mb-3">
                            Payment Summary
                        </h2>
            ...
            "
                                ...
                                <button
                                    ...
                                    className="
          mt-5 p-2 rounded-lg text-gray-400
        ...
        "
                        ...
                        <button
                            onClick={() =>
                                setExtraCharges([...extraCharges, { id: 0, name: "", price: 0, icon: undefined }])
                            }
                            className="
      w-full mt-2 py-2.5 rounded-xl border-2 border-dashed
