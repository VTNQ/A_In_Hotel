import { ArrowDown, Moon, Percent, Wallet } from "lucide-react";
import type { CreateOrUpdateTabProps } from "../../../type/promotion.types";
import { useTranslation } from "react-i18next";

const OfferTab = ({
  watch,
  setValue,
  trigger,
  errors,
}: CreateOrUpdateTabProps) => {
  const { t } = useTranslation();

  return (
    <div className="flex-1 overflow-y-auto px-10 py-10 space-y-16 bg-white dark:bg-[#0F172A] transition-colors">
      {/* OFFER */}
      <section className="space-y-8">
        <div className="flex items-center gap-4">
          <div className="h-8 w-1 bg-[#42578E] dark:bg-blue-500 rounded-full" />

          <h3 className="text-xl font-bold text-slate-800 dark:text-gray-100">
            {t("promotion.offer.title")}
          </h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* TYPE */}
          <div className="flex flex-col gap-2">
            <label className="block mb-1 font-medium text-[#253150] dark:text-gray-300">
              {t("promotion.offer.type")}
            </label>

            <div className="relative">
              <select
                value={watch("type")}
                onChange={(e) => {
                  setValue("type", e.target.value, {
                    shouldValidate: true,
                    shouldDirty: true,
                  });
                }}
                className="
                  h-12 w-full rounded-lg border px-4 pr-10 appearance-none outline-none
                  border-[#4B62A0]
                  bg-white text-gray-800
                  dark:bg-slate-800 dark:text-gray-100
                  dark:border-slate-600
                  focus:border-primary dark:focus:border-blue-500
                "
              >
                <option value="1">
                  {t("promotion.offer.fixed")}
                </option>

                <option value="2">
                  {t("promotion.offer.percent")}
                </option>

                <option value="3">
                  {t("promotion.offer.special")}
                </option>
              </select>

              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-gray-500 pointer-events-none">
                <ArrowDown />
              </span>
            </div>
          </div>

          {/* VALUE */}
          <div className="flex flex-col gap-2">
            <label className="block mb-1 font-medium text-[#253150] dark:text-gray-300">
              {t("promotion.offer.value")}
            </label>

            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-primary dark:text-blue-400">
                {watch("type") === "1" || watch("type") === "3" ? (
                  <Percent size={18} />
                ) : (
                  <Wallet size={18} />
                )}
              </span>

              <input
                type="number"
                step="0.01"
                value={watch("value")}
                onChange={(e) => {
                  setValue("value", e.target.value, {
                    shouldValidate: true,
                    shouldDirty: true,
                  });
                }}
                onBlur={() => trigger("value")}
                placeholder={
                  watch("type") === "1" || watch("type") === "3"
                    ? t("promotion.offer.valuePercentPlaceholder")
                    : t("promotion.offer.valueFixedPlaceholder")
                }
                className="
                  h-12 w-full rounded-lg border pl-12 pr-4 outline-none
                  border-[#4B62A0]
                  bg-white text-gray-800
                  dark:bg-slate-800 dark:text-gray-100
                  dark:border-slate-600
                  dark:placeholder:text-gray-400
                  focus:border-primary dark:focus:border-blue-500
                "
              />

              {errors.value && (
                <p className="text-red-500 text-sm mt-1">
                  {errors.value.message}
                </p>
              )}
            </div>
          </div>
        </div>
      </section>

      {/* CONDITIONS */}
      <section className="space-y-8">
        <div className="flex items-center gap-4">
          <div className="h-8 w-1 bg-[#42578E] dark:bg-blue-500 rounded-full" />

          <h3 className="text-xl font-bold text-slate-800 dark:text-gray-100">
            {t("promotion.conditions.title")}
          </h3>
        </div>

        <div className="grid grid-cols-1 gap-8">
          {/* MIN NIGHTS */}
          <div className="flex flex-col gap-2">
            <label className="block mb-1 font-medium text-[#253150] dark:text-gray-300">
              {t("promotion.conditions.minNights")}
            </label>

            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#253150] dark:text-gray-400">
                <Moon size={18} />
              </span>

              <input
                type="number"
                value={watch("minNights")}
                onBlur={() => trigger("minNights")}
                onChange={(e) => {
                  setValue("minNights", e.target.value, {
                    shouldValidate: true,
                    shouldDirty: true,
                  });
                }}
                placeholder={t(
                  "promotion.conditions.minNightsPlaceholder",
                )}
                className="
                  h-12 w-full rounded-lg border pl-12 pr-4 outline-none
                  border-[#4B62A0]
                  bg-white text-gray-800
                  dark:bg-slate-800 dark:text-gray-100
                  dark:border-slate-600
                  dark:placeholder:text-gray-400
                  focus:border-primary dark:focus:border-blue-500
                "
              />

              {errors.minNights && (
                <p className="text-red-500 text-sm mt-1">
                  {errors.minNights.message}
                </p>
              )}
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default OfferTab;