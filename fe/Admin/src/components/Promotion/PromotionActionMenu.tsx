...
const PromotionActionMenu :React.FC<PromotionActionMenuProps>=({
    ...
    onDiabled,
      ...
      ? [
        {label:t("promotion.view"),icon:<Eye size={16} />,onClick:()=>onView?.(promotion)},
        {label:t("common.edit"),icon:<Edit size={16} />,onClick:()=>onEdit?.(promotion)},
        {label:t("promotion.disable"),icon:<Power size={16} />,danger:true,onClick:()=>onDiabled?.(promotion)},
       ...
       return (
      <ActionMenu
        title={promotion.isActive ? t("promotion.enable") : t("promotion.disable")}
        actions={actions}
      />
    );
}
